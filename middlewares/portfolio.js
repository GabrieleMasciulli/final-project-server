...
const portfolioBalance = async (req, res, next) => {
  ...
  const holdingsPromises = await positions.map(async pos => {
    ...
    pos.transactions.map(transaction => {
      ...
      coin_holding =
        type === 'buy' ? (coin_holding += quantity) : (coin_holding -= quantity)
    })
...
module.exports = { portfolioAssets, portfolioBalance }
